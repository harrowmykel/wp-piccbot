...
class PICCBOTT_CSP4_ADMIN
{
    public $plugin_version = PICCBOTT_CSP4_VERSION;
    public $plugin_name = PICCBOTT_CSP4_PLUGIN_NAME;
    ...
    public function __construct()
    {
        if (is_admin() && (!defined('DOING_AJAX') || !DOING_AJAX)) {
            ...
            if(version_compare(phpversion(), '5.3.3', '>=')){
                add_action( 'wpms_tgmpa_register', 'picc_antibot_init_recommendations' );
            }
        }

        if (defined('DOING_AJAX')) {
            // Background API Ajax
            add_action('wp_ajax_pcbtt_csp4_backgrounds', array(&$this,'backgrounds'));
        }
    }
    ...
    {
        if (check_ajax_referer('pcbtt_csp4_backgrounds')) {
            $r = array();
            $page = '';
            ...
            if (false === (get_transient('pcbtt_csp4_backgrounds_page_'.$query.$page))) {
                //f(1){
                //echo 'miss';
                ...
                if (is_wp_error($response)) {
                    ...
                    if ($response_code == '200') {
                        set_transient('pcbtt_csp4_backgrounds_page_'.$query.$page, $response['body'], 604800);
                        echo $response['body'];
                    } else {
                ...
                //echo 'hit';
                echo get_transient('pcbtt_csp4_backgrounds_page_'.$query.$page);
            }
    ...
    {
        if (isset($_POST[ 'pcbtt_csp4_reset' ])) {
            $option_page = $_POST[ 'option_page' ];
            check_admin_referer($option_page . '-options');
            require_once(PICCBOTT_CSP4_PLUGIN_PATH.'inc/default-settings.php');

            $_POST[ $_POST[ 'option_page' ] ] = $pcbtt_csp4_settings_deafults[$_POST[ 'option_page' ]];
            add_settings_error('general', 'pcbtt_csp4-settings-reset', __("Settings reset."), 'updated');
        }
    }
    ...
    {
        if (strpos($hook_suffix, 'pcbtt_csp4') === false) {
            return;
        }
        ...
        wp_enqueue_script('pcbtt_csp4-framework-js', PICCBOTT_CSP4_PLUGIN_URL . 'framework/settings-scripts.js', array( 'jquery' ), $this->plugin_version);
        wp_enqueue_script('pcbtt_csp4-magnific-popup-js', PICCBOTT_CSP4_PLUGIN_URL . 'public/vendor/magnific-popup/jquery.magnific-popup.min.js', array( 'jquery' ), $this->plugin_version);
        wp_enqueue_script('pcbtt_csp4-tingle', PICCBOTT_CSP4_PLUGIN_URL . 'public/vendor/tingle/tingle.min.js', array(), $this->plugin_version);
        wp_enqueue_script('theme-preview');
        wp_enqueue_style('thickbox');
        wp_enqueue_style('media-upload');
        wp_enqueue_style('wp-color-picker');
        wp_enqueue_style('pcbtt_csp4-framework-css', PICCBOTT_CSP4_PLUGIN_URL . 'framework/settings-style.css', false, $this->plugin_version);
        wp_enqueue_style('font-awesome-solid', PICCBOTT_CSP4_PLUGIN_URL . 'public/vendor/fontawesome/css/solid.min.css', false, $this->plugin_version, false, $this->plugin_version);
        wp_enqueue_style('font-awesome-brands', PICCBOTT_CSP4_PLUGIN_URL . 'public/vendor/fontawesome/css/brands.min.css', false, $this->plugin_version, false, $this->plugin_version);
        wp_enqueue_style('font-awesome', PICCBOTT_CSP4_PLUGIN_URL . 'public/vendor/fontawesome/css/fontawesome.min.css', false, $this->plugin_version, false, $this->plugin_version);
        wp_enqueue_style('pcbtt_csp4-magnific-popup-js', PICCBOTT_CSP4_PLUGIN_URL . 'public/vendor/magnific-popup/magnific-popup.css', false, $this->plugin_version, false, $this->plugin_version);
        wp_enqueue_style('pcbtt_csp4-tingle-css', PICCBOTT_CSP4_PLUGIN_URL . 'public/vendor/tingle/tingle.min.css', false, $this->plugin_version, false, $this->plugin_version);
    }
    ...
    public function create_menus()
    {
        add_menu_page(
            ...
            PICCBOTT_CSP4_PLUGIN_URL . 'public/images/menu-logo.png',
            200
            );
        ...
        $plugins = pcbtt_csp4_plugins_active();
    ...
    {
        include PICCBOTT_CSP4_PLUGIN_PATH.'resources/views/stockimages.php';
    }

    public function giveaways_page()
    {
        include PICCBOTT_CSP4_PLUGIN_PATH.'resources/views/giveaways.php';
    }

    public function subscribers_page()
    {
        include PICCBOTT_CSP4_PLUGIN_PATH.'resources/views/subscribers.php';
    }

    public function addons_page()
    {
        include PICCBOTT_CSP4_PLUGIN_PATH.'resources/views/addons.php';
    }
    ...
    public function suggest_page()
    {
        ...
        window.location.href = "admin.php?page=pcbtt_csp4";
        </script>
    ...
    {
        $plugin_file = PICCBOTT_CSP4_SLUG;

        if ($file == $plugin_file) {
            $settings_link = '<a href="admin.php?page=pcbtt_csp4">Settings</a>';
    ...
    public function plugin_options_tabs()
    {
        ...
        foreach (pcbtt_csp4_get_options() as $v) {
            if ($v[ 'type' ] == 'tab') {
                $uses_tabs = true;
                break;
            }
        ...
        if ($uses_tabs) {
            ...
            foreach (pcbtt_csp4_get_options() as $v) {
                ...
                if ($menu_slug == $page && $v[ 'type' ] == 'tab') {
                    ...
                    if ($v[ 'id' ] == 'pcbtt_csp4_subscribers') {
                        echo '<a id="'.$v[ 'id' ].'" class="nav-tab ' . $active . '" href="admin.php?page=pcbtt_csp4_subscribers">';
                    } else {
                        echo '<a id="'.$v[ 'id' ].'" class="nav-tab ' . $active . '" href="?page=' . $menu_slug . '&tab=' . $v[ 'id' ] . '">';
                    }
                        
                    if ($v[ 'id' ] == 'pcbtt_csp4_setting') {
                        echo '<i class="fas fa-edit"></i> ';
                    }
                    if ($v[ 'id' ] == 'pcbtt_csp4_design') {
                        echo '<i class="fas fa-image"></i> ';
                    }
                    if ($v[ 'id' ] == 'pcbtt_csp4_subscribers') {
                        echo '<i class="fas fa-users"></i> ';
                    }
                    if ($v[ 'id' ] == 'pcbtt_csp4_advanced') {
...
<div class="wrap columns-2 pbtt-csp4">
    <div id="pcbtt_csp4_header">
        ...
        <h1>
            <img style="width:150px;margin-right:10px;margin-bottom: -2px;vertical-align: text-bottom;"
                src="<?php echo PICCBOTT_CSP4_PLUGIN_URL ?>public/images/piccbott-logo.png">
    ...
    <div class="pbtt-wrap">
        ...
        <div id="poststuff">
            ...
            do_action('pcbtt_csp4_render_page', array('tab'=>$_GET['tab']));
        } ?>
                ...
                switch ($v[ 'type' ]) {
                                            ...
                                            case 'section':
                                                ...
                                                if ($current_tab == $tab[ 'id' ] or $current_tab === false) {
                                                    if ($layout == '2-col') {
                                                        echo '<div id="'.$v[ 'id' ].'" class="postbox piccbott-postbox">';
                ...
                <div id="postbox-container-1" class="postbox-container">
                    <div id="side-sortables" class="meta-box-sortables ui-sortable">
                        ...
                        <a style="border:1px solid #ddd;display:inline-block;"
                            href="https://www.piccbott.com/landing/picc-bot-page-getting-started-video/?utm_source=picc-bot-plugin&utm_medium=banner&utm_campaign=picc-bot-banner-in-plugin"
                            target="_blank"><img
                                src="<?php echo PICCBOTT_CSP4_PLUGIN_URL; ?>framework/getting-started-banner.png" /></a>
                        <br><br>


                        <a style="border:1px solid #ddd;display:inline-block;"
                            href="<?php echo pcbtt_csp4_admin_upgrade_link('sidebar-banner')?>" target="_blank"
                            rel="noopener noreferrer" class="pbtt-csp4-cta"><img
                                src="<?php echo PICCBOTT_CSP4_PLUGIN_URL; ?>framework/picc-bot-pro-sidebar.png" /></a>
                        <br><br>
                        ...
                        <!--            <div class="postbox " style="background-color:#FAE6A4;color:#333 !important; border-color:#333 !important">
                                ...
                                <div class="inside">
                                    <div class="support-widget">
                                        ...
                                        <p>
                                        <a class="button-primary" style="background-color:#05AE0E; border-color:#05AE0E; box-shadow:none; text-shadow: none; width:100%; text-align:center;" href="<?php echo pcbtt_csp4_admin_upgrade_link('sidebar'); ?>" target="_blank" rel="noopener noreferrer">See What's In The Pro Version</a>
                                        </p>
                            ...
                            <div class="inside">
                                <div class="support-widget">
                                    ...
                                    <!--   <p style="text-align: center; margin-top:0">
                                            ...
                                            <a style="display:inline-block" href="https://wordpress.org/support/plugin/picc-bot/reviews/?filter=5#new-post">Please Rate this Plugin to show your Support!</a> <i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></p> -->
                        ...
                        <form id="pbtt-bg-form" style="display:none"
                        ...
                        <!--          <div class="postbox rss-postbox">
    											<div class="handlediv" title="Click to toggle"><br /></div>
    											<h3 class="hndle"><span><i class="fab fa-wordpress"></i>&nbsp;&nbsp;<?php _e('PiccOrg Blog', 'ultimate-picc-bot-page') ?></span></h3>
    											<div class="inside">

    												<div class="rss-widget">
    													<?php
                                                        wp_widget_rss_output(array(
                                                        'url' => 'http://feeds.feedburner.com/piccbott/',
                                                        'title' => 'PiccOrg Blog',
                                                        'items' => 3,
                                                        'show_summary' => 0,
                                                        'show_author' => 0,
...
<?php
        if (!empty($_GET['tab']) && $_GET['tab'] == 'pcbtt_csp4_design') {
            ?>
...
<script>
jQuery(document).ready(function($) {
    $(".pbtt-csp4-cta").click(function(e) {
...
<?php
    ...
    {
        foreach (pcbtt_csp4_get_options() as $k => $v) {
            switch ($v[ 'type' ]) {
                case 'menu':
                    $menu_slug = $v[ 'menu_slug' ];
    ...
    public function field_machine($args)
    {
        ...
        // Load defaults
        $defaults = array( );
        foreach (pcbtt_csp4_get_options() as $k) {
            switch ($k[ 'type' ]) {
                case 'setting':
                case 'section':
                case 'tab':
        ...
        $path = PICCBOTT_CSP4_PLUGIN_PATH . 'framework/field-types/' . $type . '.php';
        if (file_exists($path)) {
            // Show Field
            include($path);
            // Show description
                ...
                default:
                    if (!empty($k[ 'validate' ]) && $setting_id == $option_page) {
                        ...
                        foreach ($validation_rules as $v) {
                            $path = PICCBOTT_CSP4_PLUGIN_PATH . 'framework/validations/' . $v . '.php';
                            if (file_exists($path)) {
                                // Defaults Values
                                $is_valid  = true;
                                $error_msg = '';
                                ...
                                // Is it valid?
                                if ($is_valid === false) {
                                    add_settings_error($k[ 'id' ], 'piccbott_error', $error_msg, 'error');
                                    // Unset invalids
                                    unset($input[ $k[ 'id' ] ]);
                                }
                            }
    ...
    /**
     * PiccOrg version of WP's do_settings_sections
     *
     * @since 0.1.0
     */
...
add_action('admin_head', 'pcbtt_csp4_set_user_settings');
function pcbtt_csp4_set_user_settings()
{
    if (isset($_GET['page']) && $_GET['page'] == 'pcbtt_csp4') {
...
function pcbtt_csp4_plugins_active(){
...
function pcbtt_csp4_disable_admin_notices()
{
    global $wp_filter;
...
if (!empty($_GET['page']) && strpos($_GET['page'], 'pcbtt_csp4') !==  false) {
    //add_action('admin_print_scripts', 'pcbtt_csp4_disable_admin_notices');
}