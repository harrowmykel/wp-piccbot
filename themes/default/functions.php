<?php
// Template Tags
function pcbtt_csp4_title() {
	$o = pcbtt_csp4_get_settings();
	extract( $o );
...
function pcbtt_csp4_metadescription() {
	$o = pcbtt_csp4_get_settings();
	extract( $o );
...
function pcbtt_csp4_head() {
	...
	$output .= '<link rel="stylesheet" href="'.PICCBOTT_CSP4_PLUGIN_URL.'themes/default/bootstrap/css/bootstrap.min.css">'."\n";
	$output .= '<link rel="stylesheet" href="'.PICCBOTT_CSP4_PLUGIN_URL.'themes/default/style.css">'."\n";
	if ( is_rtl() ) {
		$output .= '<link rel="stylesheet" href="'.PICCBOTT_CSP4_PLUGIN_URL.'themes/default/rtl.css">'."\n";
	}
    ...
    <?php if ( !empty( $text_font ) ):?>
	    .pbtt-csp4 body{
	        font-family: <?php echo PICCBOTT_CSP4::get_font_family($text_font); ?>
	    }

	    .pbtt-csp4 h1, .pbtt-csp4 h2, .pbtt-csp4 h3, .pbtt-csp4 h4, .pbtt-csp4 h5, .pbtt-csp4 h6{
	        font-family: <?php echo PICCBOTT_CSP4::get_font_family($text_font); ?>
	    }
    <?php endif;?>
    ...
    <?php if ( !empty( $link_color ) ) { ?>
		.pbtt-csp4 a, .pbtt-csp4 a:visited, .pbtt-csp4 a:hover, .pbtt-csp4 a:active, .pbtt-csp4 a:focus{
	...
	$output .= '<script src="'.PICCBOTT_CSP4_PLUGIN_URL.'themes/default/bootstrap/js/bootstrap.min.js"></script>'."\n";
...
function pcbtt_csp4_logo() {
	...
	if ( !empty( $logo ) ) {
		$output .= "<img id='pbtt-csp4-image' src='$logo'>";
	}

	return  $output;
}

function pcbtt_csp4_headline() {
	$o = pcbtt_csp4_get_settings();
	extract( $o );
	...
	if ( !empty( $headline ) ) {
		$output .= '<h1 id="pbtt-csp4-headline">'.$headline.'</h1>';
	}

	return  $output;
}

function pcbtt_csp4_description() {
	$o = pcbtt_csp4_get_settings();
	extract( $o );
	...
	if ( !empty( $description ) ) {
		if(has_shortcode( $description,'rafflepress')){
			$output .= '<div id="pbtt-csp4-description">'.do_shortcode(shortcode_unautop(wpautop(convert_chars(wptexturize($description))))).'</div>';
		}else{
			$output .= '<div id="pbtt-csp4-description">'.shortcode_unautop(wpautop(convert_chars(wptexturize($description)))).'</div>';
		}
...
function pcbtt_csp4_append_html() {
	$o = pcbtt_csp4_get_settings();
	extract( $o );
	...
	if ( !empty( $append_html) ) {
		$output .= '<div id="picc-bot-custom-html">'.$append_html.'</div>';
	}

	return  $output;
}

function pcbtt_csp4_credit() {
	$o = pcbtt_csp4_get_settings();
	extract( $o );
