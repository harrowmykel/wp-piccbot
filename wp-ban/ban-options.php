...
if( ! empty( $_POST['Submit'] ) ) {
    ...
    if(!empty($banned_ips_post)) {
        foreach($banned_ips_post as $banned_ip) {
            if($admin_login == 'admin' && ($banned_ip == ban_get_ip() || is_admin_ip($banned_ip))) {
                $text .= '<p style="color: blue;">'.sprintf(__('This IP \'%s\' Belongs To The Admin And Will Not Be Added To Ban List', 'wp-piccbot'),$banned_ip).'</p>';
            } else {
        ...
        foreach( $banned_ips_range_post as $banned_ip_range ) {
            ...
            if( sizeof( $range ) === 2 ) {
                ...
                if( $admin_login === 'admin' && ( check_ip_within_range( ban_get_ip(), $range_start, $range_end ) ) ) {
                    $text .= '<p style="color: blue;">'.sprintf( __( 'The Admin\'s IP \'%s\' Fall Within This Range (%s - %s) And Will Not Be Added To Ban List', 'wp-piccbot' ), ban_get_ip(), $range_start, $range_end ).'</p>';
                } else {
    ...
    if(!empty($banned_hosts_post)) {
        foreach($banned_hosts_post as $banned_host) {
            if($admin_login == 'admin' && ($banned_host == @gethostbyaddr(ban_get_ip()) || is_admin_hostname($banned_host))) {
                $text .= '<p style="color: blue;">'.sprintf(__('This Hostname \'%s\' Belongs To The Admin And Will Not Be Added To Ban List', 'wp-piccbot'), $banned_host).'</p>';
            } else {
    ...
    if(!empty($banned_referers_post)) {
        foreach($banned_referers_post as $banned_referer) {
            if(is_admin_referer($banned_referer)) {
                $text .= '<p style="color: blue;">'.sprintf(__('This Referer \'%s\' Belongs To This Site And Will Not Be Added To Ban List', 'wp-piccbot'), $banned_referer).'</p>';
            } else {
    ...
    if(!empty($banned_user_agents_post)) {
        foreach($banned_user_agents_post as $banned_user_agent) {
            if(is_admin_user_agent($banned_user_agent)) {
                $text .= '<p style="color: blue;">'.sprintf(__('This User Agent \'%s\' Is Used By The Current Admin And Will Not Be Added To Ban List', 'wp-piccbot'), $banned_user_agent).'</p>';
            } else {
    ...
    $update_ban_text[] = __( 'Banned Options', 'wp-piccbot' );
    $update_ban_text[] = __('Banned IPs', 'wp-piccbot');
    $update_ban_text[] = __('Banned IP Range', 'wp-piccbot');
    $update_ban_text[] = __('Banned Host Names', 'wp-piccbot');
    $update_ban_text[] = __('Banned Referers', 'wp-piccbot');
    $update_ban_text[] = __('Banned User Agents', 'wp-piccbot');
    $update_ban_text[] = __('Banned Excluded IPs', 'wp-piccbot');
    $update_ban_text[] = __('Banned Message', 'wp-piccbot');
    $i=0;
    foreach($update_ban_queries as $update_ban_query) {
        if($update_ban_query) {
            $text .= '<p style="color: green;">'.$update_ban_text[$i].' '.__('Updated', 'wp-piccbot').'</p>';
        }
        $i++;
    }
    if(empty($text)) {
        $text = '<p style="color: red;">'.__('No Ban Option Updated', 'wp-piccbot').'</p>';
    }
}
if( ! empty( $_POST['do'] ) ) {
    // Decide What To Do
    switch( $_POST['do'] ) {
        ...
        case __('Reset Ban Stats', 'wp-piccbot'):
            ...
            if( ! empty( $_POST['reset_ban_stats'] ) &&$_POST['reset_ban_stats'] === 'yes' ) {
                ...
                $text = '<p style="color: green;">'.__('All IP Ban Stats And Total Ban Stat Reseted', 'wp-piccbot').'</p>';
            } else {
                ...
                update_option('banned_stats', $banned_stats);
                $text = '<p style="color: green;">'.__('Selected IP Ban Stats Reseted', 'wp-piccbot').'</p>';
            }
            break;
    }
}
...
/* <![CDATA[*/
    ...
    function banned_default_templates(template) {
        ...
        switch(template) {
            case "message":
                default_template = "<!DOCTYPE html>\n<html>\n<head>\n<meta charset=\"utf-8\">\n<title>%SITE_NAME% - %SITE_URL%</title>\n</head>\n<body>\n<div id=\"wp-ban-container\">\n<p style=\"text-align: center; font-weight: bold;\"><?php _e('You Are Banned.', 'wp-piccbot'); ?></p>\n</div>\n</body>\n</html>";
                break;
        }
    ...
    jQuery(document).ready(function() {
        jQuery('#show_button').click(function(event)
        {
            ...
            {
                jQuery(this).val('<?php _e('Show Current Banned Message', 'wp-piccbot'); ?>');
                jQuery('#banned_preview_message').empty();
                jQuery(banned_template_message_el).fadeIn('fast');
            }
            else
            {
                jQuery(this).val('<?php _e('Show Banned Message Template', 'wp-piccbot'); ?>');
...
<div class="wrap">
    <h2><?php _e('Ban Options', 'wp-piccbot'); ?></h2>
    <table class="widefat">
        <thead>
            <tr>
                <th><?php _e('Your Details', 'wp-piccbot'); ?></th>
                <th><?php _e('Value', 'wp-piccbot'); ?></th>
            </tr>
        </thead>
        <tr>
            <td><?php _e('IP', 'wp-piccbot'); ?>:</td>
            <td><strong><?php echo ban_get_ip(); ?></strong></td>
        </tr>
        <tr class="alternate">
            <td><?php _e('Host Name', 'wp-piccbot'); ?>:</td>
            <td><strong><?php echo @gethostbyaddr(ban_get_ip()); ?></strong></td>
        </tr>
        <tr>
            <td><?php _e('User Agent', 'wp-piccbot'); ?>:</td>
            <td><strong><?php echo $_SERVER['HTTP_USER_AGENT']; ?></strong></td>
        </tr>
        <tr class="alternate">
            <td><?php _e('Site URL', 'wp-piccbot'); ?>:</td>
            <td><strong><?php echo get_option('home'); ?></strong></td>
        </tr>
        <tr>
            <td valign="top" colspan="2" align="center">
                <?php _e('Please <strong>DO NOT</strong> ban yourself.', 'wp-piccbot'); ?>
            </td>
        </tr>
    </table>
    <p>&nbsp;</p>
    <table class="form-table">
        <tr>
            <td width="40%" valign="top">
                <strong><?php _e('Reverse Proxy Check', 'wp-piccbot'); ?>:</strong><br />
                <?php _e( 'This will assume that incoming requests include the user\'s IP address in the HTTP_X_FORWARDED_FOR (and the request IP will be from your proxy).', 'wp-piccbot' ); ?>
            </td>
            <td width="60%">
                <label>
                    ...
                    <?php _e( 'I am using a reverse proxy.', 'wp-piccbot' ); ?>
                </label>
                <p>
                    <?php _e( 'If you\'re not sure, leave this unchecked. Ticking this box when you don\'t have a reverse proxy will make it easy to bypass the IP ban.', 'wp-piccbot' ); ?>
                </p>
            </td>
        </tr>
        <tr>
            <td valign="top">
                <strong><?php _e('Banned IPs', 'wp-piccbot'); ?>:</strong><br />
                <?php _e('Use <strong>*</strong> for wildcards.', 'wp-piccbot'); ?><br />
                <?php _e('Start each entry on a new line.', 'wp-piccbot'); ?><br /><br />
                <?php _e('Examples:', 'wp-piccbot'); ?>
        ...
        <tr>
            <td valign="top">
                <strong><?php _e('Banned IP Range', 'wp-piccbot'); ?>:</strong><br />
                <?php _e('Start each entry on a new line.', 'wp-piccbot'); ?><br /><br />
                <?php _e('Examples:', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <span dir="ltr">192.168.1.1-192.168.1.255</span><br /><br />
                <?php _e('Notes:', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <?php _e('No Wildcards Allowed.', 'wp-piccbot'); ?><br />
            </td>
            <td>
        ...
        <tr>
            <td valign="top">
                <strong><?php _e('Banned Host Names', 'wp-piccbot'); ?>:</strong><br />
                <?php _e('Use <strong>*</strong> for wildcards', 'wp-piccbot'); ?>.<br />
                <?php _e('Start each entry on a new line.', 'wp-piccbot'); ?><br /><br />
                <?php _e('Examples:', 'wp-piccbot'); ?>
        ...
        <tr>
            <td valign="top">
                <strong><?php _e('Banned Referers', 'wp-piccbot'); ?>:</strong><br />
                <?php _e('Use <strong>*</strong> for wildcards', 'wp-piccbot'); ?>.<br />
                <?php _e('Start each entry on a new line.', 'wp-piccbot'); ?><br /><br />
                <?php _e('Examples:', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <span dir="ltr">http://*.blogspot.com</span><br /><br />
                <?php _e('Notes:', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <?php _e('There are ways to bypass this method of banning.', 'wp-piccbot'); ?>
            </td>
            <td>
        ...
        <tr>
            <td valign="top">
                <strong><?php _e('Banned User Agents', 'wp-piccbot'); ?>:</strong><br />
                <?php _e('Use <strong>*</strong> for wildcards', 'wp-piccbot'); ?>.<br />
                <?php _e('Start each entry on a new line.', 'wp-piccbot'); ?><br /><br />
                <?php _e('Examples:', 'wp-piccbot'); ?>
                ...
                <?php _e('Suggestions:', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <?php _e('See <a href="http://www.user-agents.org/">http://www.user-agents.org/</a>', 'wp-piccbot'); ?>
            </td>
            <td>
        ...
        <tr>
            <td valign="top">
                <strong><?php _e('Banned Exclude IPs', 'wp-piccbot'); ?>:</strong><br />
                <?php _e('Start each entry on a new line.', 'wp-piccbot'); ?><br /><br />
                <?php _e('Examples:', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <span dir="ltr">192.168.1.100</span><br /><br />
                <?php _e('Notes:', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <?php _e('No Wildcards Allowed.', 'wp-piccbot'); ?><br />
                <strong>&raquo;</strong> <?php _e('These Users Will Not Get Banned.', 'wp-piccbot'); ?>
            </td>
            <td>
        ...
        <tr>
            <td valign="top">
                <strong><?php _e('Banned Message', 'wp-piccbot'); ?>:</strong><br /><br /><br />
                    <?php _e('Allowed Variables:', 'wp-piccbot'); ?>
                    ...
                    <p><?php printf(__('Note: Your message must be within %s', 'wp-piccbot'), htmlspecialchars('<div id="wp-ban-container"></div>')); ?></p><br />
                    <input type="button" name="RestoreDefault" value="<?php _e('Restore Default Template', 'wp-piccbot'); ?>" onclick="banned_default_templates('message');" class="button" /><br /><br />
                    <input type="button" id="show_button" value="<?php _e('Show Current Banned Message', 'wp-piccbot'); ?>" class="button" /><br />
            </td>
            <td>
    ...
    <p style="text-align: center;">
        <input type="submit" name="Submit" class="button" value="<?php _e('Save Changes', 'wp-piccbot'); ?>" />
    </p>
</div>
</form>
<p>&nbsp;</p>
...
<div class="wrap">
    <h3><?php _e('Ban Stats', 'wp-piccbot'); ?></h3>
    <br style="clear" />
    <table class="widefat">
        <thead>
            <tr>
                <th width="40%" style="text-align: center;"><?php _e('IPs', 'wp-piccbot'); ?></th>
                <th width="30%" style="text-align: center;"><?php _e('Attempts', 'wp-piccbot'); ?></th>
                <th width="30%"><input type="checkbox" id="toogle_checkbox" name="toogle_checkbox" value="1" onclick="toggle_checkbox();" />&nbsp;<label for="toogle_checkbox"><?php _e('Action', 'wp-piccbot'); ?></label></th>
            </tr>
        </thead>
            <?php
                // Credits To Joe (Ttech) - http://blog.fileville.net/
                if(!empty($banned_stats['users'])) {
                    ...
                    foreach($banned_stats['users'] as $key => $value) {
                        ...
                        echo "<td><input type=\"checkbox\" id=\"ban-$i\" name=\"delete_ips[]\" value=\"$key\" />&nbsp;<label for=\"ban-$i\">".__('Reset this IP ban stat?', 'wp-piccbot')."</label></td>\n";
                        echo '</tr>'."\n";
                        $i++;
                    }
                } else {
                    echo "<tr>\n";
                    echo '<td colspan="3" align="center">'.__('No Attempts', 'wp-piccbot').'</td>'."\n";
                    echo '</tr>'."\n";
                }
            ?>
        <tr class="thead">
            <td style="text-align: center;"><strong><?php _e('Total  Attempts:', 'wp-piccbot'); ?></strong></td>
            <td style="text-align: center;"><strong><?php echo number_format_i18n(intval($banned_stats['count'])); ?></strong></td>
            <td><input type="checkbox" id="reset_ban_stats" name="reset_ban_stats" value="yes" />&nbsp;<label for="reset_ban_stats"><?php _e('Reset all IP ban stats and total ban stat?', 'wp-piccbot'); ?></label></td>
        </tr>
    </table>
    <p style="text-align: center;"><input type="submit" name="do" value="<?php _e('Reset Ban Stats', 'wp-piccbot'); ?>" class="button" onclick="return confirm('<?php _e('You Are About To Reset Ban Stats.', 'wp-piccbot'); ?>\n\n<?php _e('This Action Is Not Reversible. Are you sure?', 'wp-piccbot'); ?>')" /></p>
</div>
</form>
<p>&nbsp;</p>
